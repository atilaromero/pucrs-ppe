...
namespace comm {
	...
	public:
		...
		T popNow(bool *empty, T fallbackValue) {
			std::unique_lock<std::mutex> lock(this->d_mutex);
			*empty = this->d_queue.empty();
			if (*empty) {
...
void multiply(unsigned int threads){
	comm::queue<long int> *lines = new comm::queue<long int>;
	...
	for (int t = 0; t<threads; t++){
		th.push_back(std::thread ([lines]{
			bool empty = 0;
			for(long int i = lines->popNow(&empty,-1); !empty; i = lines->popNow(&empty,-1)){
